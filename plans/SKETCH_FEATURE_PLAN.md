...
| 11 | **Gallery dispatch + routing** | `NotesGallery.qml` delegate + `ADD-SKETCH-*` states, `NotesGalleryNarrowToolbar.qml` submenu, `TripPage.qml` sketch page registration | `tst_NotesGallerySketch` |

**Properties of this split:**

- **Commits 1–7 ship zero UI.** The entire data + persistence + pipeline foundation is mergeable and reviewable before any visible feature exists. Each lands independently green.
...
- **Commits 9–11 are feature-visible.** Each adds a user-testable slice: sketch renders → creatable via menu → navigable from gallery. Bisect-friendly if regressions surface.
- **Commit 10 (Add-note menu) has no hard dependency on sketches** — it could land before commit 7 — but putting it after commit 7 lets its `Sketch` menu item actually create something, rather than sit as dead UI.
