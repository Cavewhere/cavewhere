...
TEST_CASE("Test cwAngleInput setting and getting", "[cwAngleInput]") {
    ...
    rows << Row("4 %", "4 %", cwUnits::Degrees, 0.0, QValidator::Invalid, false);
    ...
    foreach(const Row& row, rows) {
        ...
        CHECK(angle.value(cwUnits::Degrees, row.Unit) == Approx(row.ValueDouble));
