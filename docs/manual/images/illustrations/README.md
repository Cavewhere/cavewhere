...
| `break-the-tie-in.svg` | Hand-authored for `loop-closure/check-loop-closure.md` — the two-panel version of the break-the-tie-in technique. Left: trip 2's last station was typed `A2`, so cavern honors the declared join and compresses that run to reach `A2`. Right: renamed `A2x`, the junction breaks, the run stays anchored at `A1` and swings out to where its own shots put it, and the free end lands nearest `A4`. The "nearest" label is drawn true: from the free end at (96,214) the distances are 30 px to A4, 115 px to A3 and 148 px to A2, so A4 really is the closest station in the figure. The **left panel's compression is schematic** — the direction is right (a gross topology error makes the adjustment shorten that run to reach the station you named) but the 23% shortening is drawn for legibility, not solved. Same palette and plain hand-written SVG as `setting-*.svg`. |
| `setting-*.svg` | Hand-authored for `scraps/warping-settings.md` — one diagram per warping setting, drawn from the behavior in `cavewherelib/src/cwTriangulateTask.{h,cpp}`. Plain hand-written SVG, no editor or toolchain: edit the XML directly. Colors are the manual's own tokens (ink `#171b20`, accent `#17618f`, mark `#d6552b`, muted `#5c656e`) on a white card, matching the `projection-*` figures and staying legible in either page theme. |
