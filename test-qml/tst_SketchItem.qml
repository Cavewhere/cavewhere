...
Item {
    ...
    TestCase {
        ...
        function test_eraserDragMergesIntoOneUndoStep() {
            resetToolbarToPen()
            const strokesBefore = sketchId.strokeCount()
            drawStroke([Qt.point(250, 200), Qt.point(350, 200),
                        Qt.point(450, 200), Qt.point(550, 200)])
            tryVerify(() => sketchId.strokeCount() === strokesBefore + 1,
                      2000, "Setup stroke should exist")
            const strokesAfterDraw = sketchId.strokeCount()
            ...
            tryVerify(() => sketchId.strokeCount() !== strokesAfterDraw,
        ...
        function test_stylusInputAndUndo() {
            const initial = sketchId.strokeCount()
            ...
            drawStroke([Qt.point(200, 100),
                        ...
                        Qt.point(300, 140),
                        Qt.point(350, 160)])
            tryVerify(() => sketchId.strokeCount() === initial + 1,
                      2000, "First drag should produce a stroke")
            // The stroke's captured points are asserted in C++
            // (test_cwSketch.cpp); QML only observes the stroke count.

            drawStroke([Qt.point(300, 100), Qt.point(400, 200)])
            tryVerify(() => sketchId.strokeCount() === initial + 2,
