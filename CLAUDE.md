...
- **Keep the production public API free of test scaffolding.** Do not add `*ForTesting` methods, `m_*ForTesting` member state, or `friend class FooTestAccess` declarations to production classes (`cwProject`, `cwSaveLoad`, `cwSaveLoadPrivate`, etc.) just to make a test observable. Tests must verify behavior through the same surface real callers see: end-to-end disk-state checks (file existence, byte content, mtime snapshots), existing public signals (`objectPathReady`, `saveFlushCompleted`, `discardCompleted`), and the standard wait helpers (`waitSaveToFinish`, `waitLoadToFinish`, `waitForDiscardToFinish`). Existing test seams like `cwSaveLoad::waitForFinished()` and `cwProject::waitSaveToFinish()` are pre-existing exceptions; do not add new ones. If a corruption test needs to "fail at the queue level before filesystem damage", accept the one-step-later disk-state signal instead — it still fails the test, just after the bytes land. The `waitForFinish()`/`waitToFinish()` helpers on managers (e.g. `cwProject`, `cwScrapManager`) listed earlier as test-only exist solely for test use and predate this rule; don't add more.
