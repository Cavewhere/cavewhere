...
public slots:
    cwScreenCaptureCommand* dequeueScreenCaptureCommand();
    void enqueueSceenCaptureCommand(cwScreenCaptureCommand* command);
...
private:
    ...
    QVector<cwCaptureViewport*> Captures;
    QVector<cwCaptureItem*> Layers;

    //Queue of all the commands generated by cwCaptureViewports
    //This queue is processed by Qt3D and CaptureTileFrameNode
    QQueue<cwScreenCaptureCommand*> ScreenCaptureCommands;
...
/**
 * @brief cwCaptureManager::isScreenCaptureCommandQueueEmpty
 * @return True if the screen capture command queue is empty, and false if it still has commands
 */
inline bool cwCaptureManager::isScreenCaptureCommandQueueEmpty() const
{
    return ScreenCaptureCommands.isEmpty();
}
