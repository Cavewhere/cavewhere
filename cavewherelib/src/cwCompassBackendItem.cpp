...
cwCompassBackendItem::cwCompassBackendItem(QQuickItem* parent)
    : QQuickRhiItem(parent)
{
    ...
    double offset = 0.63;
    m_labelNorthPosition.setBinding([this, offset]() {
        return computeLabelPosition(QVector3D(0.0f, offset * 1.40, 0.0f));
    });
    m_labelSouthPosition.setBinding([this, offset]() {
        return computeLabelPosition(QVector3D(0.0f, -offset, 0.0f));
    });
    m_labelEastPosition.setBinding([this, offset]() {
        return computeLabelPosition(QVector3D(offset, 0.0f, 0.0f));
    });
    m_labelWestPosition.setBinding([this, offset]() {
        return computeLabelPosition(QVector3D(-offset, 0.0f, 0.0f));
    });

    m_updateNotifier = m_rotation.addNotifier([this]() {
        update();
    });
}

QPointF cwCompassBackendItem::computeLabelPosition(const QVector3D& pos)
{
    // Apply rotation
    QMatrix4x4 modelMatrix;
    modelMatrix.rotate(m_rotation.value());

    QVector3D rotatedPos = modelMatrix.map(pos);
    ...
    return QPointF(rotatedPos.x() * halfWidth + halfWidth,
                   -rotatedPos.y() * halfHeight + halfHeight);
}

// cwCamera* cwCompassItem::camera() const
// {
//     return m_camera;
// }
...
// QQuaternion cwCompassItem::rotation() const
// {
//     return m_rotation;
// }

// void cwCompassBackendItem::setRotation(const QQuaternion& rotation)
// {
...
QQuickRhiItemRenderer* cwCompassBackendItem::createRenderer()
{
    return new cwCompassRenderer();
}

// void cwCompassBackendItem::componentComplete()
// {
//     QQuickRhiItem::componentComplete();

//     auto mThis = this;

//     m_updateNotifier = m_rotation.onValueChanged([this]() {
//         // qDebug() << "Update changed!" << this;
//         this->update();
//     });
// }
...
//     vertexXShader->setSourceFile(":/shaders/compass/compassShadowX.vert");

//     cwGLShader* fragmentShader = new cwGLShader(QOpenGLShader::Fragment);
//     fragmentShader->addDefine("HORIZONTAL_BLUR_9");
//     fragmentShader->setSourceFile(":/shaders/compass/compassShadow.frag");
...
//     vertexYShader->setSourceFile(":/shaders/compass/compassShadowX.vert");

//     cwGLShader* fragmentYShader = new cwGLShader(QOpenGLShader::Fragment);
//     fragmentYShader->addDefine("VERTICAL_BLUR_9");
//     fragmentYShader->setSourceFile(":/shaders/compass/compassShadow.frag");
...
//     vertexShader->setSourceFile(":/shaders/compass/compassShadowOutput.vert");

//     cwGLShader* fragmentShader = new cwGLShader(QOpenGLShader::Fragment);
//     fragmentShader->setSourceFile(":/shaders/compass/compassShadowOutput.frag");
...
//     //Setup the normal shader
//     cwGLShader* vertexShader = new cwGLShader(QOpenGLShader::Vertex);
//     vertexShader->setSourceFile(":/shaders/compass/compass.vert");

//     cwGLShader* fragmentShader = new cwGLShader(QOpenGLShader::Fragment);
//     fragmentShader->setSourceFile(":/shaders/compass/compass.frag");
