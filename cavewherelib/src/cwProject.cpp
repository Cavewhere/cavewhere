...
QFuture<ResultBase> cwProject::loadHelper(QString filename, LoadParameters params)
{
    ...
    if (params.knownType == UnknownFileType) {
        ...
        return AsyncFuture::observe(typeFuture)
            .context(this, [this, filename, typeFuture, params]() {
                // Call loadHelperImpl (not loadHelper) to avoid re-entering detection
                // when the detected type happens to be UnknownFileType
                LoadParameters resolvedParams = params;
                resolvedParams.knownType = typeFuture.result();
                return loadHelperImpl(filename, resolvedParams);
            }).future();
    }

    return loadHelperImpl(filename, params);
}

QFuture<ResultBase> cwProject::loadHelperImpl(const QString& filename, LoadParameters params)
{
    const FileType type = params.knownType;
    const bool suppressLoadedEmit = params.suppressLoadedEmit;
                        ...
                        = AsyncFuture::observe(filenameFuture)
                              .context(this, [saveLoad, filenameFuture, this]() {
                                  ...
                                  return Monad::mbind(filenameFuture, [this](const Monad::ResultString& filename) {
                                      return loadHelper(filename.value(), {GitFileType, true});
                                  });
                              }).future();
